...
if __name__ == '__main__':
    ...
    for file_name in data_path:
        ...
        pred = pred_data['pred']
        original_data = read_ply(os.path.join(original_data_dir, file_name.replace('/', '\\').split('\\')[-1][:-4] + '.ply'))
        ...
        print(str(file_name.replace('/', '\\').split('\\')[-1][:-4]) + '_acc:' + str(correct / float(len(labels))))
